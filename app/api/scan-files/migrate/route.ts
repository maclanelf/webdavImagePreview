...
export async function POST(request: NextRequest) {
  try {
    ...
    if (migrateAll) {
      // 迁移所有缓存
      const result = scanFiles.migrateAllFromCache()
    ...
    // 迁移单个缓存
    const result = scanFiles.migrateFromCache(cacheId)
    return NextResponse.json(result)
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 获取所有缓存及其迁移状态
    const caches = scanCache.getAll() as any[]
    ...
    const status = caches
      ...
      .map(cache => {
        const hasData = scanFiles.hasData(cache.id)
        const stats = hasData ? scanFiles.getStats(cache.id) : null
